...
Your code runs natively from `~/ros2_ws`. Why it is set up this way is on the
...
The interfaces repo is the only RAMMP source you build. It is here so your
node compiles against the same message types that are inside the images.
...
sheppy launches packages through their launch files, so add one now even
though the node is a single executable. `rammp_hello/launch/hello.launch.py`:
...
So far sheppy runs the stable modules and you run your node by hand. Adding
your node to the manifest makes it part of the system: it starts and stops
with everything else, appears in `sheppy status`, and other people can bring
it up without knowing how. A `launch_file` alternative runs `ros2 launch`
from your workspace, so every restart runs your latest build.
